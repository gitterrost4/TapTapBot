...
public class HeroListener extends AbstractMessageListener {
  ...
  public HeroListener(JDA jda) {
    ...
    ConnectionHelper.update(
        "create table if not exists hero (id INTEGER PRIMARY KEY not null, name text not null UNIQUE, emoji text, imageurl text, skill1name text, skill1desc text, skill2name text, skill2desc text, skill3name text, skill3desc text, skill4name text, skill4desc text, maxhp integer, attack integer, speed integer, defense integer, uppullrate integer);");
  }
  ...
  protected void messageReceived(MessageReceivedEvent event, CommandMessage messageContent) {
    ...
    case "add":
      ...
      ConnectionHelper.update(
          "insert into hero (name, emoji, imageurl, skill1name,skill1desc, skill2name,skill2desc, skill3name, skill3desc, skill4name, skill4desc, maxhp, attack, speed, defense,uppullrate) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
